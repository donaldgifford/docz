...
## Developing docz-api, the server

docz-api's own development guide, folded in by IMPL-0019 Phase 2 with its
headings demoted one level. Its recipes are the `api` just module: read each
`just <recipe>` below as `just api <recipe>`, `Dockerfile` as `Dockerfile.api`,
`deploy/` as `deploy/api/`, and `docs/` as `docs/archive/api/`.
