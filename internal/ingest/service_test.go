...
// TestBuildDocumentsReportsOnlyDoczFilenames pins the IMPL-0009 follow-up: an
// enabled api: block widens the fetch to every .md under docs_dir, so page
// candidates reach buildDocuments too. Only a file whose name follows the
// docz convention is worth reporting when it has no frontmatter — a type
// dir's own README.md is regenerated by every `docz update`, and a genuine
// stray is already reported once by buildPages.
func TestBuildDocumentsReportsOnlyDoczFilenames(t *testing.T) {
	cfg, err := loadConfig([]byte(fixtureConfig))
	if err != nil {
	...
	docs, err := buildDocuments(&cfg, []BlobEntry{
		{Path: "docs/frameworks/README.md", Content: []byte("# Frameworks\n")},   // docz-generated: silent
		{Path: "docs/frameworks/notes.md", Content: []byte("# Stray\n")},         // buildPages reports it: silent here
		{Path: "docs/frameworks/0002-nofm.md", Content: []byte("# No matter\n")}, // looks like a doc: reported
	})
	if err != nil {
		t.Fatalf("buildDocuments: %v", err)
	}
	if len(docs) != 0 {
		t.Fatalf("docs = %+v, want none (no frontmatter anywhere)", docs)
	}
	if !slices.Equal(w.paths, []string{"docs/frameworks/0002-nofm.md"}) {
		t.Errorf("warned paths = %v, want only the docz-named file", w.paths)
	}
}
