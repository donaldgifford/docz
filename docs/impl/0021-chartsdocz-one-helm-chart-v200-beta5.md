...
- [x] `README.md.gotmpl`: a title, a badges line, and the helm-docs values
  table. Phase 4 writes the prose
  Done; `README.md` generated by helm-docs.
- [x] `tests/chart/helpers_test.yaml`, with no templates yet: a suite over a
  ...
  Done over a throwaway `templates/tests/helpers-probe.yaml`, gated on `helpersProbe`: 6 tests covering release `docz`, release `prod`, `fullnameOverride`, the role label, `extraLabels` appended, and a collision failing.
