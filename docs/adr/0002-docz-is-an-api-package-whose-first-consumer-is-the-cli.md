...
1. **docz is an API package; the CLI is its first consumer.** Every
   operation the CLI performs is reachable as a Go API with typed results.
   `cmd/` is flags → one call → print. The test of the shape is that each
   command reduces to a single call, and anything a command does beyond that
   line is a gap in the library, not a feature of the shell.
...
3. **One module.** `github.com/donaldgifford/docz` stays a single module
   with one version line. Go compiles per imported package, so a consumer of
   ...
   ADR-0001 Alternative B stays rejected.
...
6. **Build the whole API, then swap `cmd/`.** The unit of delivery is the
   ...
   benchmark, still binding). That PR is the `minor` that releases
   everything.
...
7. **Supersession and the freeze.** This ADR supersedes ADR-0001 Decision 2's
   ...
   packages. It **does not apply** to packages added under this ADR until the
   `cmd/` swap lands: they are marked experimental, may change between beta
   pins without a major bump, and freeze when the swap ships. From that
   release forward this ADR governs the whole surface and ADR-0001 is
   superseded in part.
   ...
   stateDiagram-v2
     ...
     Experimental --> Experimental: change between beta pins
     Experimental --> Frozen: cmd swap PR ships (minor)
...
| Decision 6 — v1.0.0 freeze | Kept for the five packages; **does not apply** to new packages before the swap |
...
- **Release mechanics are unchanged.** `dont-release` for additive landings,
  one `minor` for the swap; IMPL-0017's `updated:` field retargets to the
  release after it.
...
- a. **Pseudo-versions, no tags.** `go get github.com/donaldgifford/docz@<sha>`
  ...
  *(recommendation)*
- b. Manual `v1.3.0-beta.N` tags on `main` merge commits via `make release
  TAG=…` — readable, but confirm first that `pr-semver-bump` does not adopt a
  pre-release tag as the base for the next `minor`.
